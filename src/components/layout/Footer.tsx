    ...
    <ul className="space-y-2.5">
      {items.map((item) => (
        <li key={item.href + item.label}>
          <Link
            to={item.href}
            className="inline-block py-0.5 text-sm text-gray-400 transition-colors hover:text-brand-300"
          >
            {item.label}
          </Link>
        </li>
  ...
  return (
    <footer className="bg-navy-950 text-gray-400">
      ...
      <div className="border-b border-white/5 bg-gradient-to-r from-brand-600 to-brand-500">
        <AnimateIn animation="fade-up" className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      ...
      <div className="border-b border-white/5 bg-navy-900/60">
        <div className="mx-auto grid max-w-7xl gap-4 px-4 py-6 sm:grid-cols-2 sm:gap-5 sm:px-6 sm:py-8 lg:grid-cols-4 lg:px-8">
          {trustFeatures.map((f, index) => {
            ...
            return (
              ...
              <div className="flex items-center gap-3 rounded-xl bg-white/[0.02] p-3 sm:bg-transparent sm:p-0">
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
        <div className="lg:grid lg:grid-cols-4 lg:gap-10">
          ...
          <p className="mt-2 max-w-sm text-sm leading-relaxed text-gray-500">
            Custom software, AI solutions, and cloud-native systems for modern businesses.
          </p>
          <div className="mt-5">
          ...
          <FooterAccordion title="Our Services">
            <LinkList items={services.map((s) => ({ label: s.title, href: '/services' }))} />
          </FooterAccordion>

          <FooterAccordion title="Contact & Products">
            ...
            <p className="mb-3 mt-6 text-xs font-bold uppercase tracking-wider text-white md:mt-8">
              Our Products
            </p>
            <LinkList items={products.map((p) => ({ label: p.title, href: '/products' }))} />
          </FooterAccordion>
          </div>
        </div>
      </div>
