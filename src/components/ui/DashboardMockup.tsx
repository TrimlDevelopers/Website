export default function DashboardMockup() {
  return (
    <div className="animate-float relative mx-auto w-full max-w-md sm:max-w-lg lg:max-w-xl">
            ...
            <div className="col-span-12 p-2.5 sm:col-span-9 sm:p-3">
              <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4 sm:gap-2">
                {[
                  ...
                  <div key={m.label} className="rounded-lg border border-slate-200 bg-white p-1.5 sm:p-2">
                    <p className="truncate text-[7px] text-slate-400 sm:text-[8px]">{m.label}</p>
                    <p className="text-xs font-bold text-brand-500 sm:text-sm">{m.value}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-5">
                <div className="rounded-lg border border-slate-200 bg-white p-2 sm:col-span-3">
                  <p className="text-[8px] font-medium text-slate-500">Maintenance Overview</p>
                  <div className="mt-2 flex h-10 items-end gap-1 sm:h-12">
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-2 sm:col-span-2">
                  <p className="text-[8px] font-medium text-slate-500">Asset Health</p>
                  <div className="relative mx-auto mt-1 h-10 w-10 sm:h-12 sm:w-12">
                    ...
                    <span className="absolute inset-0 flex items-center justify-center text-[8px] font-bold text-brand-500 sm:text-[9px]">
                      88%
                    </span>
                  </div>
                  <p className="text-center text-[8px] text-emerald-500">Good</p>
                </div>
              </div>
            </div>
          </div>
